...
def test_gemini_path_called_when_key_present(fresh_insight_cache, monkeypatch):
    ...
    assert out["positioning"] == "MOCK_INSIGHT"
    assert mock_client.generate.call_count == 4   # structural + curve + flow + positioning
...
def test_cache_hit_within_5_min_skips_gemini(fresh_insight_cache, monkeypatch):
    ...
    insights.generate_insights(_row())
    assert mock_client.generate.call_count == 4, "second call must hit cache, not Gemini"
