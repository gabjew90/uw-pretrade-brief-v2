...
def stub_uw(monkeypatch):
    """Patch every storage.fetch_* to return a minimal happy payload."""
    monkeypatch.setattr(storage, "fetch_flow_alerts",
                        lambda limit=100: {"data": [{"ticker": t} for t in
                                                     ["NVDA", "TSLA", "AMD", "PLTR", "AMC",
                                                      "AAPL", "GOOGL", "MSFT", "META", "NFLX",
                                                      "AMZN", "F", "BAC", "WMT", "JPM"]]})
    monkeypatch.setattr(storage, "fetch_spot_exposures_strike",
                        lambda t, is_hot=False: {"data": [{"strike": 100.0,
                                                            "call_gamma_oi": 1.0,
                                                            "put_gamma_oi": 0.5,
                                                            "spot_price": 100.0}]})
    monkeypatch.setattr(storage, "fetch_oi_strike",
                        lambda t, is_hot=False: {"data": [{"strike": 100,
                                                            "open_interest": 1000,
                                                            "prev_open_interest": 800}]})
    monkeypatch.setattr(storage, "fetch_volatility",
                        lambda t, is_hot=False: {"data": [{"dte": 7, "iv": 0.4},
                                                           {"dte": 30, "iv": 0.45}]})
    monkeypatch.setattr(storage, "fetch_interpolated_iv",
                        lambda t, is_hot=False: {"data": [{"iv_rank": 42}]})
    monkeypatch.setattr(storage, "fetch_darkpool",
                        lambda t, is_hot=False: {"data": [{"price": 100, "size": 1000}]})
    monkeypatch.setattr(storage, "fetch_earnings",
                        lambda t, is_hot=False: {"data": [{"report_date": "2026-07-15"}]})
...
async def test_refresh_snapshot_per_endpoint_failure_partial_row(stub_uw, fresh_storage_state,
    ...
    failure = storage.UWFailure(endpoint="darkpool", ticker="NVDA", message="503")
    def patched(t, is_hot=False):
        if t == "NVDA":
            return failure
        return {"data": [{"price": 100, "size": 1000}]}
    monkeypatch.setattr(storage, "fetch_darkpool", patched)
