...
- **Click = full build.** Clicking a (light) ticker builds the full row on demand via `/api/lookup` → `build_single_row` (all 4 gates + tile inputs + the deep-dive **verdict**), then Tile 3-rich / Tile 4 fetch on demand. Re-clicks are cache-cheap. Every UW call traces to a page-load or a click.
- **Deep-dive verdict** (`server/verdict.py`): a 3-leg read shown atop the deep-dive — **Positioning** (Flow+OI collapsed; green only on opening-flow), **Structural** (gamma), and **Skew** (25Δ risk-reversal from UW's vendor endpoint, sign-corrected; greeks-derived fallback; an asymmetric oppose-veto — never a peer green), with Cost as a guard, a `signal_conflict` banner, and an **action** headline ("Worth acting on" / "Skip — signals disagree" / "Wait" / "Stand down"). Deep-dive only; the grid stays light.
