...
- **`server/verdict.py`** (Plan 3) — pure deep-dive `row.verdict`: collapses Flow+OI into **Positioning** (green only on opening_flow; total_flow caps yellow; archive-decoupled; unwinding caps), **Structural**, and **Skew** (25Δ risk-reversal DERIVED from `greeks` — RR₂₅ = 25Δ-call IV − 25Δ-put IV, sign by construction; asymmetric oppose-veto, agree is subordinate, never a peer green), Cost demoted to a guard, `signal_conflict` + an `action` headline. Computed in `build_single_row` (skew from a ~30d greeks expiry, `_skew_expiry`); **deep-dive only** (None on light grid rows). Rendered by `renderVerdictPanel`.
